...
"""Ingestor for intraday equity and index market data."""
...
class EquityIntradayIngestor(BaseIngestor):
    """Pull intraday candles from configured data sources and persist them."""
    ...
    def fetch_raw(
        ...
        **kwargs: Any,
        ...
        """Fetch raw intraday candles for a symbol."""
        self._last_symbol = symbol
        if "raw" in kwargs:
            return kwargs["raw"]

        start = start or datetime.utcnow() - timedelta(days=5)
    ...
    def normalize(self, raw: Any) -> List[Candle]:
        """Normalize provider-specific payloads into Candle objects."""
        ...
        if not symbol:
            raise ValueError("Symbol required to normalize candles")
